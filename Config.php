...
class Config
{
    const TABLE_SQLS = [
        ...
        ServiceAbstract::class => "CREATE TABLE IF NOT EXISTS Services (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, service VARCHAR(20) UNIQUE NOT NULL, cost VARCHAR(20) NOT NULL, deadline VARCHAR(20))",
        ProductAbstract::class => "CREATE TABLE IF NOT EXISTS Products (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, name VARCHAR(40), cost VARCHAR(20), manufacture VARCHAR(40), date VARCHAR(30), user_email VARCHAR(30), service VARCHAR(30), FOREIGN KEY (user_email) REFERENCES Users(email), product VARCHAR(30), FOREIGN KEY (service) references Services(service))",
